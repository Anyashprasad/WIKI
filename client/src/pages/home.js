...
export default function Home() {
    ...
    return (<div className="min-h-screen bg-black text-white relative overflow-x-hidden">
      {/* Header */}
      <header className="relative z-10 px-6 py-8">
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <img src="/logo.svg" alt="WIKI Security Scanner" className="h-12 w-auto" onError={(e) => {
        ...
        <motion.div className="max-w-4xl mx-auto text-center" variants={containerVariants} initial="hidden" animate="visible">
          ...
          <motion.div variants={itemVariants} className="max-w-2xl mx-auto mb-12">
            <div className="flex gap-3">
              <Input value={input} onChange={(e) => setInput(e.target.value)} onKeyPress={handleKeyPress} placeholder="Enter a URL to scan for vulnerabilities..." className="flex-1 bg-gray-800/50 border-gray-700 text-white placeholder-gray-400 h-14 text-lg backdrop-blur-sm" disabled={scanMutation.isPending}/>
              <Button onClick={handleSubmit} disabled={!input.trim() || scanMutation.isPending} className="bg-green-400 text-black hover:bg-green-300 h-14 px-8 font-semibold transition-all duration-200 hover:scale-105">
