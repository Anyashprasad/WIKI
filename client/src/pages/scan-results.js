...
import { ArrowLeft, Download, FileText, Shield, Database, Code, ShieldAlert, Bug, Zap } from "lucide-react";
...
export default function ScanResults() {
    ...
    const { data: initialScan, isLoading } = useQuery({
    ...
    const displayVulnerabilities = ((progress?.status === 'completed' && progress?.vulnerabilities)
    ...
    const vulnerabilities = displayVulnerabilities || [];
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-3">
              <img src="/logo.svg" alt="WIKI Security Scanner" className="h-8 w-auto" onError={(e) => {
          ...
          <div className="flex items-center space-x-4">
            <Button variant="outline" onClick={() => exportMutation.mutate("json")} disabled={exportMutation.isPending || currentStatus !== 'completed'} className="border-gray-700 hover:border-green-400 text-white bg-transparent hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed">
              <Download className="h-4 w-4 mr-2"/>
              Export JSON
            </Button>
            <Button variant="outline" onClick={() => exportMutation.mutate("excel")} disabled={exportMutation.isPending || currentStatus !== 'completed'} className="border-gray-700 hover:border-green-400 text-white bg-transparent hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed">
              <FileText className="h-4 w-4 mr-2"/>
              Export Excel
            </Button>
            <Button variant="outline" onClick={() => exportMutation.mutate("pdf")} disabled={exportMutation.isPending || currentStatus !== 'completed'} className="border-gray-700 hover:border-green-400 text-white bg-transparent hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed">
              <FileText className="h-4 w-4 mr-2"/>
              Export PDF
            </Button>
          </div>
      ...
      <main className="px-6 py-8">
        <div className="max-w-6xl mx-auto space-y-8">
          ...
          {((currentStatus === "scanning" || currentStatus === "crawling" || currentStatus === "pending") ||
            (currentStatus === "completed" && progress?.vulnerabilitiesFound > 0 && vulnerabilities.length === 0)) && (<Card className="bg-gray-900 border-gray-700">
                ...
                currentStatus === "pending" ? "Initializing scan..." :
                  ...
                  {currentStage && (<Badge variant="outline" className="mb-6 border-green-400/30 text-green-400 px-4 py-1 text-base">
                      {currentStage}
                    </Badge>)}

                  <div className="w-full max-w-xl mx-auto bg-gray-800 rounded-full h-4 overflow-hidden">
                    <div className="bg-green-400 h-full rounded-full transition-all duration-500 ease-out relative" style={{ width: `${Math.max(5, currentProgress)}%` }}>
          ...
          {currentStatus === "completed" && !(progress?.vulnerabilitiesFound > 0 && vulnerabilities.length === 0) && (<>
              ...
              <Card className="bg-gray-900 border-gray-700">
                ...
                <CardContent>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {Object.entries(severityCounts).map(([severity, count]) => (<div key={severity} className="text-center">
              ...
              {/* Charts */}
              {vulnerabilities.length > 0 && (<Card className="bg-gray-900 border-gray-700">
                  <CardHeader>
              ...
              <Card className="bg-gray-900 border-gray-700">
                ...
                <CardContent className="space-y-6">
                  {vulnerabilities.length === 0 ? (<div className="text-center py-8">
                    ...
                    </div>) : (vulnerabilities.map((vulnerability) => {
                const Icon = categoryIcons[vulnerability.category] || Bug;
                return (<Card key={vulnerability.id} className="bg-gray-800 border-gray-600">
                          <CardHeader>
                            <div className="flex items-center justify-between">
                              ...
                              <Button onClick={() => setLocation(`/fix/${vulnerability.id}`)} className="bg-green-400 text-black hover:bg-green-300 hover:scale-105 transition-all duration-200" data-testid={`button-generate-fix-${vulnerability.id}`}>
                                View Fix Details
                              </Button>
                            </div>
                          </CardHeader>
          ...
          {/* ERROR STATE */}
          {(currentStatus === "failed" || currentStatus === "error") && (<Card className="bg-red-900/20 border-red-500/50">
              <CardContent className="py-12 text-center">
                ...
                <Button onClick={() => setLocation("/")} className="mt-6 bg-red-500 hover:bg-red-600 text-white">
                  Return Home
                </Button>
              </CardContent>
            </Card>)}
