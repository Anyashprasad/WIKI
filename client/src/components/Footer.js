...
export default function Footer() {
    ...
    const getLetterIntensity = (index) => {
        if (!isHovering || letterPositions.length === 0)
            return 0;
        const letterPos = letterPositions[index];
        if (!letterPos)
            return 0;
        const distance = Math.sqrt(Math.pow(mousePos.x - letterPos.x, 2) +
    ...
    return (<>
      ...
      <AnimatePresence>
        {showGiantFooter && (<motion.div initial={{ y: '100vh' }} animate={{ y: 0 }} exit={{ y: '100vh' }} transition={{ duration: 0.8, ease: [0.25, 0.1, 0.25, 1] }} className="fixed bottom-0 left-0 w-full h-screen bg-gray-900 z-50 flex flex-col pointer-events-auto">
            {/* Top section */}
            <div className="px-8 py-6 grid grid-cols-1 md:grid-cols-3 items-center border-b border-gray-800 gap-4 md:gap-0">
              ...
              <div className="flex justify-center md:justify-end">
                <button onClick={scrollToTop} className="text-sm text-gray-400 hover:text-white transition-colors duration-200 flex items-center gap-2 hover:bg-gray-800 px-3 py-1 rounded">
                  Back to top
            ...
            <div className="flex-1 bg-green-400 flex items-center justify-center">
              <div ref={containerRef} className="cursor-default select-none" onMouseMove={handleMouseMove} onMouseEnter={() => setIsHovering(true)} onMouseLeave={() => setIsHovering(false)}>
                <div className="flex items-center justify-center">
                  {letters.map((letter, index) => (<motion.span key={index} className="interactive-letter text-black font-black text-8xl md:text-9xl lg:text-[12rem] xl:text-[15rem] leading-none" style={getGlowStyle(getLetterIntensity(index))} initial={{ scale: 0, opacity: 0, rotateY: 90 }} animate={{ scale: 1, opacity: 1, rotateY: 0 }} transition={{
                    duration: 0.6,
                    delay: 0.3 + (index * 0.1),
                    ease: "backOut"
                }}>
